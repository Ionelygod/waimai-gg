  ...
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span class="block" :class="{active:selectType === 0}" @click="setType(0)">
        推荐<span class="count">{{RatingPleasedCount}}</span>
      </span>
      <span class="block" :class="{active:selectType === 1}" @click="setType(1)">
        吐槽<span class="count">{{dissatisfiedCount}}</span>
      </span>
    </div>
...
export default {
  ...
  methods:{
    ...
    toggle(){
      this.$emit('toggleRatingCountent')
    }
  },
  computed:{
    ...mapGetters(['RatingCount', 'RatingPleasedCount', 'dissatisfiedCount'])
  }
}
</script>
